                ...
                <v-card>
                  <v-card-text>
                    <v-container>
                      ...
                      <TitleAndSubtitle title="Agora, informe seus
                            dados pessoais" sub="Seus dados são salvos automaticamente." />
                      <v-row>
                        <!-- 987.098.098-88 -->
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="CPF" variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.cpf" name="cpf"
                            @blur="saveField($event.target)" autofocus v-mask-cpf></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="Data de Nasc." variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.birthDate"
                            name="birthDate" @blur="saveField($event.target)" v-mask="'00/00/0000'"></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="Identidade" variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.identity"
                            name="identity" @blur="saveField($event.target)"></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="Orgão Emissor" variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.organ"
                            name="organConsignor" @blur="saveField($event.target)"></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="Nacionalidade" variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.nationality"
                            name="nationality" @blur="saveField($event.target)"></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-text-field label="Naturalidade" variant="underlined"
                            class="mt-1 block w-full border-gray-500 text-xs md:text-base" v-model="state.naturality"
                            name="naturality" @blur="saveField($event.target)"></v-text-field>
                        </v-col>
                        <v-col cols="6" sx="6" sm="6" md="3">
                        ...
                        <v-col cols="6" sx="6" sm="6" md="3">
                          <v-select class="m-1" variant="underlined" label="Nº de Dependentes" v-model="state.dependents"
                            :items="['0', '1', '2', '3', '4+']" name="dependents" @blur="saveField($event.target)">
                          </v-select>
                        </v-col>
                        <v-col cols="12" sx="12" sm="12" md="4">
                          <v-select class="m-2" variant="underlined" label="Grau de Instrução"
                            v-model="state.degreeEducation" :items="[
                              ...
                              'Pós-graduação',
                              'Mestrado',
                              'Doutorado']" name="EducationLevel" @blur="saveField($event.target)">
                          </v-select>
                        </v-col>
                        <v-col cols="12" sx="12" sm="12" md="4">
                          <v-select class="m-2" variant="underlined" label="Sexo" v-model="state.sex" :items="[
                            'Masculino',
